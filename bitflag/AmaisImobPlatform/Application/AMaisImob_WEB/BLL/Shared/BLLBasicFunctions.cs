...
namespace BLL
{
    public abstract class BLLBasicFunctions<TEntity, TViewModel> : IBLLBasicFunctions<TEntity, TViewModel>
        where TEntity : class
        where TViewModel : class

    {
        protected AMaisImob_DB.Models.AMaisImob_HomologContext context;
        ...
        public BLLBasicFunctions(AMaisImob_DB.Models.AMaisImob_HomologContext context, string entityIdPropertyName)
        {
        ...
        public IQueryable<TEntity> GetDataAsNoTracking(Expression<Func<TEntity, bool>> _where) => (from l in this.dbSet select l).Where(_where).AsNoTracking();
        public IQueryable<TEntity> GetDataAsNoTracking() => (from l in this.dbSet select l).AsNoTracking();
        ...
        public IQueryable<TEntity> GetDataFiltered(DataTablesAjaxPostModel filter, out int recordsTotal, out int recordsFiltered, string whereSQL, SqlParameter[] whereParameters)
        {
            ...
            return this.dbSet.FromSql<TEntity>(sql.ToString().Replace(guid, "*"), parameters.ToArray()).AsNoTracking();
        }
        ...
        public virtual int CreateOrUpdate(TViewModel model, string entityIdPropertyName = null)
        {
            var id = (int?)model.GetType().GetProperty(string.IsNullOrWhiteSpace(entityIdPropertyName) ? this.entityIdPropertyName : entityIdPropertyName).GetValue(model, null);
            if (!id.HasValue) return Create(model);
