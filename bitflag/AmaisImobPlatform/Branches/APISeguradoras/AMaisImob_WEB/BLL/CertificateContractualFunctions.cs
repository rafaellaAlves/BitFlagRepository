...
using AMaisImob_WEB.BLL.GuarantorIntegration;
...
namespace AMaisImob_WEB.BLL
{
    public class CertificateContractualFunctions : BLLBasicFunctions<CertificateContractual, CertificateContractualViewModel>
    {
        ...
        public async Task<BLL.GuarantorIntegration.GuarantorProvider> GetGuarantorProvider(int certificateContractualId)
        {
            var guarantor = await (from cc in context.CertificateContractual
                                   join cgpt in context.CategoryGuarantorProductTax on cc.CategoryGuarantorProductTaxId equals cgpt.CategoryGuarantorProductTaxId
                                   join g in context.Guarantor on cgpt.GuarantorId equals g.GuarantorId
                                   where cc.CertificateContractualId == certificateContractualId
                                   select g).SingleAsync();


            if (guarantor == null || string.IsNullOrWhiteSpace(guarantor.ExternalCode)) return GuarantorIntegration.GuarantorProvider.None;

            switch (guarantor.ExternalCode.ToUpper())
            {
                case "POTTENCIAL":
                    return GuarantorIntegration.GuarantorProvider.Pottencial;
                default:
                    return GuarantorIntegration.GuarantorProvider.None;
            }
        }
    }
}
