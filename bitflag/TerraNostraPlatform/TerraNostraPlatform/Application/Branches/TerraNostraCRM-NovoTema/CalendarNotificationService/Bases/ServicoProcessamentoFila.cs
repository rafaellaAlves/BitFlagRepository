...
namespace BackgroundServices.Bases
{
    public abstract class ServicoProcessamentoFila<T> : ServicoProcessamento, IFilaProcessamento<T> where T : class
    {
        ...
        public ServicoProcessamentoFila(int intervalo)
            : base(intervalo)
        {
            this.DefinirMaxItemsParalelos(1);
        }
        public ServicoProcessamentoFila(int intervalo, Dictionary<string, object> parametros)
            : base(intervalo, parametros)
        {
            this.DefinirMaxItemsParalelos(1);
        }
