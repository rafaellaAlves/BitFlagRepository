...
namespace GuardMedWeb.Controllers
{
    public class SeguroProfissionalController : Controller
    {
        ...
        public SeguroProfissionalController(DAL.GuardMedWebContext context, ICompositeViewEngine viewEngine, MailFunctions mailFunctions, IConfiguration configuration)
        {
            this._context = context;
            this.systemVariableFunctions = new BLL.System.SystemVariableFunctions(context);
            this.seguradoProfissionalFunctions = new BLL.SeguradoProfissional.SeguradoProfissionalFunctions(context);
        ...
        public async Task<IActionResult> LockedPage(string reference)
        {
            ...
            if (seguradoProfissional.PlanoSeguroProfissionalId.HasValue)
            {
                ...
                var precos = especialidadePrecoFunctions.GetDataViewModel(especialidadePrecoFunctions.GetData().Single(x => x.Gropo == grupo && x.PlanoSeguroProfissionalId == seguradoProfissional.PlanoSeguroProfissionalId));
        ...
        public async Task<IActionResult> Consulta(int? seguradoProfissionalId, int? discount, string medicGroupDiscount)
        {
            ...
            var precos = especialidadePrecoFunctions.GetData().Where(x => x.Gropo == grupo).ToList();
            ...
            //precos.ForEach(x => {
            //    x.PrecoAdmAnual *= medicGroup != null ? (float)(100 - medicGroup.Discount / 100) : 1;
            //    x.PrecoAdmMensal *= medicGroup != null ? (float)(100 - medicGroup.Discount) / 100 : 1;
            //    x.PrecoAnual *= medicGroup != null ? (float)(100 - medicGroup.Discount) / 100 : 1;
            //    x.PrecoMensal *= medicGroup != null ? (float)(100 - medicGroup.Discount) / 100 : 1;
            //    });
        ...
        //#region [STEP 2.3]
        //[HttpGet]
        //[ActionName("CotacaoFinal")]
        //public async Task<IActionResult> CotacaoFinal(string reference)
        //{
        //    Models.SeguradoProfissionalViewModel model = null;
        //    var r = GetSeguradoProfissionalViewModel(reference, ref model);
        //    if (r != null) return await Task.Run<IActionResult>(() => { return r; });
        ...
        //    var grupo = ObterGrupo(model.SeguradoProfissionalId.Value);
        //    //ViewData["Especialidades"] = especialidadeProfissionalFunctions.GetData().Where(x => x.SeguradoProfissionalId == model.SeguradoProfissionalId).ToList();
        //    var precos = especialidadePrecoFunctions.GetDataViewModel(especialidadePrecoFunctions.GetData().Single(x => x.Gropo == grupo && x.PlanoSeguroProfissionalId == model.PlanoSeguroProfissionalId));
        //    ViewData["Precos"] = precos;
        //    ViewData["PlanoSeguroProfissional"] = planoSeguroProfissionalFunctions.GetData().Where(x => x.PlanoSeguroProfissionalId == model.PlanoSeguroProfissionalId).First();
        ...
        //    if (medicGroups != null && medicGroups.Count > 0)
        //        medicGroup = medicGroups.OrderByDescending(x => x.Discount).First();
        //    var desconto = (medicGroup != null ? (100 - medicGroup.Discount) / 100 : 1);

        //    model.PrecoMensalDoPlano = (precos.PrecoMensal * desconto).ToString("#,##0.00");
        //    ViewData["desconto"] = desconto;

        //    return await Task.Run<IActionResult>(() => { return View(model); });
        //}
        //[HttpPost]
        //[ActionName("CotacaoFinal")]
        //public async Task<IActionResult> _CotacaoFinal(string reference)
        //{
        //    Models.SeguradoProfissionalViewModel model = null;
        //    var r = GetSeguradoProfissionalViewModel(reference, ref model);
        //    if (r != null) return await Task.Run<IActionResult>(() => { return r; });
        ...
        public async Task<IActionResult> _Formulario(string reference, Models.SeguradoProfissionalViewModel model, [FromForm] IFormFile file)
        {
            ...
            //var file = HttpContext.Request.Form.Files["Arquivo"];
        ...
        public async Task<IActionResult> _Pagamento(string reference, int? PagamentoTipoId, int? vezesPagamento)
        {
            ...
            var medicGroups = medicGroupFunctions.GetMedicGroupByInsurance(seguradoProfissionalViewModel.SeguradoProfissionalId.Value);
            #endregion
            ...
            var precoSeguroRCP = especialidadePrecoFunctions.GetData(x => x.Gropo == grupoSeguroRCP && x.PlanoSeguroProfissionalId == seguradoProfissionalViewModel.PlanoSeguroProfissionalId).Single();
            #endregion
            ...
            seguradoProfissionalFunctions.UpdatePagamento(seguradoProfissionalViewModel.SeguradoProfissionalId.Value, PagamentoTipoId.Value, PagamentoTipoId == 1 ? 1 : 12, precoTotalComDescontoSeguroRCP);
        ...
        public async Task<IActionResult> _Pagamento(List<string> references)
        {
            foreach (var reference in references)
            {
                ...
                seguradoProfissionalFunctions.UpdatePagamento(model.SeguradoProfissionalId.Value, 1, 12, precos.PrecoAnual);
                ...
                _model.PrecoMensalDoPlano = precos._PrecoMensal;
                _model.PrecoTotal = precos.PrecoAnual;
        ...
        //public async Task<IActionResult> SendMail(string reference, int? id)
        //{
        //    foreach (var references in reference)
        //    {

        //        Models.SeguradoProfissionalViewModel model = null;
        //        var _model = seguradoProfissionalFunctions.GetDataViewModel(seguradoProfissionalFunctions.GetDataByID(id));

        //        var password = await seguradoProfissionalFunctions.GetAndGenerateNewPassword(id.Value);
        //        var attachments = new List<System.Net.Mail.Attachment>();
        ...
        //        if (!string.IsNullOrWhiteSpace(_model.RetroatividadeFileName))
        //        {
        //            var directory = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "RetroatividadeSegurado", _model.RetroatividadeFileName);
        //            attachments.Add(new System.Net.Mail.Attachment(docDirectory, System.Net.Mime.MediaTypeNames.Application.Octet));
        //        }

        //        await mailFunctions.SendAsync("Bem vindo a GuardMed! Certificado Individual de Proteção", PagamentoEmailMessage(_model, password), new List<string>() { "joao.gregorio@bitflag.systems" }, attachments, false);
        //    }
        //    return await Task.Run<IActionResult>(() => { return Json(true); });
        //}
        ...
        public async Task<IActionResult> PagamentoCartaoDeCredito(string reference, string token)
        {
            ...
            var precos = especialidadePrecoFunctions.GetData().Single(x => x.Gropo == grupo && x.PlanoSeguroProfissionalId == seguradoProfissional.PlanoSeguroProfissionalId);
            ...
            if (!string.IsNullOrWhiteSpace(token)) seguradoProfissional.IUGU_token = token;

            // STEP0 - Custom API IUGU
            var iugu = new BLL.IUGU.IUGU(this.tokenIUGU, this.IUGUID, configuration);

            if (string.IsNullOrWhiteSpace(seguradoProfissional.IUGU_customer_id))
            {
                ...
                seguradoProfissional.IUGU_customer_id = (iuguUser.Data["id"] as string);
            }
            #endregion

            if (string.IsNullOrWhiteSpace(seguradoProfissional.IUGU_customer_payment_method_id) || seguradoProfissional.RenovacaoPagamentoBloqueado)
            {
                #region [STEP2 - CRIAR FORMA DE PAGAMENTO PARA USUARIO]
                BLL.IUGU.IUGU.ResponseData iuguCustomerDefaultPaymentMethod;
                try
                {
                    iuguCustomerDefaultPaymentMethod = await iugu.AddCustomerPaymentMethod(seguradoProfissional.IUGU_customer_id, "Cartão de Crédito - Padrão", seguradoProfissional.IUGU_token, true);
                    if (iuguCustomerDefaultPaymentMethod.Status != System.Net.HttpStatusCode.OK)
                    {
                        if (!seguradoProfissional.EmProcessoDeRenovacao)
                            await iugu.RemoveCustomer(seguradoProfissional.IUGU_customer_id);
                ...
                seguradoProfissional.IUGU_customer_payment_method_id = (iuguCustomerDefaultPaymentMethod.Data["id"] as string);
                #endregion
            }
            ...
            try
            {
                ...
                iuguSubscription = await iugu.AddSubscription(precos.CodIUGU.Trim(), seguradoProfissional.IUGU_customer_id, "credit_card", desconto, model.DescontoPlataforma.HasValue ? $"Desconto" : (medicGroup != null ? $"Desconto do grupo médico '{medicGroup.MedicGroupName}'." : ""), protecaoRenda, coberturaAdicionalDiretorChefeEquipe, coberturaAdicionalSocioEmpresaAreaMedica, expires_at);
                if (iuguSubscription.Status != System.Net.HttpStatusCode.OK)
                {
                    if (seguradoProfissional.RenovacaoPagamentoBloqueado)
                        await iugu.RemoveCustomerPaymentMethod(seguradoProfissional.IUGU_customer_id, seguradoProfissional.IUGU_customer_payment_method_id);
            ...
            seguradoProfissional.IUGU_subscription_id = (iuguSubscription.Data["id"] as string);
            #endregion
        ...
        public async Task<IActionResult> PagamentoBoletoBancario(string reference)
        {
            ...
            var seguroProfissional = seguroProfissionalPrecoFunctions.GetData(x => x.PlanoSeguroProfissionalId == model.PlanoSeguroProfissionalId && x.IdadeMin <= model.Idade && model.Idade <= x.IdadeMax).First();
            var grupo = await seguradoProfissionalFunctions.ObterGrupo(model.SeguradoProfissionalId.Value);
            var precos = especialidadePrecoFunctions.GetData().Single(x => x.Gropo == grupo && x.PlanoSeguroProfissionalId == model.PlanoSeguroProfissionalId);
            var medicGroups = medicGroupFunctions.GetMedicGroupsByCRM(model.CRM, model.EstadoCRM);
            ...
            seguradoProfissional.IUGU_invoice_id = (iuguInvoice.Data["id"] as string);
            seguradoProfissional.IUGU_invoice_secure_url = (iuguInvoice.Data["secure_url"] as string);
            #endregion
        ...
        //public async Task UpdateDataCompra()
        //{
        //    var segurados = this._context.SeguradoProfissional.Where(x => !string.IsNullOrWhiteSpace(x.IUGU_customer_id) && !x.IsDeleted).ToList();

        //    var iugu = new BLL.IUGU.IUGU(this.tokenIUGU, this.IUGUID);

        //    foreach (var item in segurados)
        //    {
        //        var customer = await iugu.GetCustomer(item.IUGU_customer_id);

        //        if (customer.Status != System.Net.HttpStatusCode.OK)
        //        {
        //            item.DataCompra = item.DataAtualizacao;
        //        }
        //        else
        //        {
        //            item.DataCompra = (DateTime)customer.Data["created_at"];
        //        }
