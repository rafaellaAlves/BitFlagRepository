...
namespace WEB.Controllers
{
    ...
    public class ReportController : Controller
    {
        ...
        public async Task<IActionResult> List(Models.DataTablesAjaxPostModel filter, int status, int? productName, int? realEstateAgencyId, int? realEstateId, string startDate, string endDate, int? motivo, int? origemId, int? situacaoId)
        {
            ...
            {
                double? plataformaComissao = null;
                var plataforma = companies.SingleOrDefault(x => x.CompanyId == item.PlataformaId);
                if (plataforma != null) plataformaComissao = plataforma.Comissao;
                // recalcular com comissão do grupo médico
                if (item.GrupoMedicoPlataformaId.HasValue)
                {
                    plataforma = companies.SingleOrDefault(x => x.CompanyId == item.GrupoMedicoPlataformaId);
                    plataformaComissao = item.GrupoMedicoPlataformaComissao;
                }

                double? escritorioComissao = null; 
                var escritorio = companies.SingleOrDefault(x => x.CompanyId == item.EscritorioId);
                if (escritorio != null) escritorioComissao = escritorio.Comissao;
                // recalcular com comissão do grupo médico
                if (item.GrupoMedicoEscritorioId.HasValue)
                {
                    escritorio = companies.SingleOrDefault(x => x.CompanyId == item.GrupoMedicoEscritorioId);
                    escritorioComissao = item.GrupoMedicoEscritorioComissao;
                }
                ...
                {
                    if (plataforma != null && plataformaComissao.HasValue)
                    {
                        if (status == (int)InsuranceStatus.New)
                        {
                            item.ComissaoAno = item.ValorTotalAnualComDesconto / 100d * (plataformaComissao);
                        }
                        else
                        {
                            item.ComissaoAno = item.PrecoTotal / 100d * (plataformaComissao);
                        }
                    }
                ...
                else if (User.IsInRole("Corretor"))
                {
                    ...
                    if (plataforma != null && plataformaComissao.HasValue)
                    {
                        if (status == (int)InsuranceStatus.New)
                        {
                            if (escritorio != null)
                            {
                                item.ComissaoAno = item.ValorTotalAnualComDesconto / 100d * (plataformaComissao - escritorioComissao);
                            }
                            else
                            {
                                item.ComissaoAno = item.ValorTotalAnualComDesconto / 100d * (plataformaComissao);
                        ...
                        else
                        {
                            if (escritorio != null)
                            {
                                item.ComissaoAno = item.PrecoTotal / 100d * (plataformaComissao - escritorioComissao);
                            }
                            else
                                item.ComissaoAno = item.PrecoTotal / 100d * (plataformaComissao);
                        }
                ...
                {
                    if (escritorio != null && escritorioComissao.HasValue)
                    {
                        if (status == (int)InsuranceStatus.New)
                        {
                            item.ComissaoAno = item.ValorTotalAnualComDesconto / 100d * escritorioComissao;
                        }
                        else
                        {
                            item.ComissaoAno = item.PrecoTotal / 100d * escritorioComissao;
                        }
                    }
                    else
                    {
        ...
        public async Task<IActionResult> LoadApoliceViewComponent(int seguradoProfissionalId) => await Task.Run(() => ViewComponent(typeof(ViewComponents.Apolice.ApoliceViewComponent), new { seguradoProfissionalId }));
