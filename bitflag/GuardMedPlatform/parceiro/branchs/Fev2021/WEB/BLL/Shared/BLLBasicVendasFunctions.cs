...
namespace BLL
{
    public abstract class BLLBasicVendasFunctions<TEntity, TViewModel> : IBLLBasicFunctions<TEntity, TViewModel>
        where TEntity : class
        where TViewModel : class

    {
        protected VendasContext context;
        ...
        public BLLBasicVendasFunctions(VendasContext context, string entityIdPropertyName)
        {
            ...
            foreach (var property in this.context.Model.FindEntityType(typeof(TEntity).FullName).GetProperties())
            {
                if (property.Name.ToUpper().Equals(entityIdPropertyName.ToUpper()))
                {
                    sqlIdPropertyName = this.context.Model.FindEntityType(typeof(TEntity).FullName).FindProperty(property.Name).Relational().ColumnName;
                    break;
                }
            }
        }
