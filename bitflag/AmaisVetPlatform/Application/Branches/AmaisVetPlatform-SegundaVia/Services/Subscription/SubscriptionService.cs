...
namespace Services.Subscription
{
    public class SubscriptionService
    {
        ...
        public async Task<int> GetSubscriptionId(SubscriptionViewModel model)
        {
            return (await context.Subscription.SingleAsync(x => x.FullName == model.FullName && x.BirthDate == model.BirthDate && x.Crmv == model.Crmv && x.Cpf == model.Cpf && x.BirthDate == model.BirthDate)).SubscriptionId;
        }
        public async Task<SubscriptionViewModel> GetLastFinishedSubscriptionAsync(string fullName, string crmv, DateTime birthDate, string cpf)
        {
            var subscription = (await context.Subscription.Where(x => x.FullName == fullName && x.Crmv == crmv && x.Cpf == cpf && x.BirthDate == birthDate && x.HasCertificate).ToListAsync()).LastOrDefault();
        ...
        public async Task<SubscriptionViewModel> GetSubscriptionViewModelAsync(int subscriptionId)
        {
        ...
        {
            var subscription = await context.Subscription.SingleOrDefaultAsync(x => x.SubscriptionId == subscriptionId);
