...
namespace Services.Plan
{
    public class PlanService
    {
        ...
        public PlanService(ApplicationDbContext.Context.ApplicationDbContext context, IMemoryCache memoryCache,  PlanCoverageTypeService planCoverageTypeService)
        {
        ...
        public async Task<string> GetPlainPaymentGatewayExternalCode2Async(int? planId, int? planPriceTableId)
        {
        ...
        public async Task<List<PlanInfoViewModel>> GetPlainInfoAsync()
        {
            return await (from p in context.Plan
                          where p.IsActive
                          orderby p.Name ascending
                          select new PlanInfoViewModel
                          {
                              PlanId = p.PlanId,                       
                              Name = p.Name,                             
                          }).ToListAsync();
        }
        public async Task<List<PlanInfoViewModel>> GetPlainInfoPriceAsync()
        {
            return await (from pp in context.PlanPriceTableEntry
                          orderby pp.PlanPriceTableId ascending
                          select new PlanInfoViewModel
                          {
                              PlanId = pp.PlanId,                              
                              MonthlyCost = pp.MonthlyCost,
                          }).ToListAsync();
        }
