...
namespace WEB.Controllers
{
    ...
    public class Subscription : Controller
    {
        ...
        public async Task<List<PlanCoverageTypeViewModel>> GetActivePlanCoverageTypesCacheAsync()
        {
        ...
        public async Task<List<PlanInfoViewModel>> GetActivePlansInfoCacheAsync(int planPriceTableId)
        {
        ...
        public async Task<IActionResult> Start(SubscriptionViewModel model)
        {
            ...
            try
            {
                using (var transaction = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    ...
                    await mailService.SendAsync("Contato", "Cotação – Acesso aos planos da AmaisVet Seguros Veterinários", await GetHtmlFromView("~/Views/Mail/Subscription/Start.cshtml", subscriptionViewModel), subscriptionViewModel.Email);
        ...
        #region [Info]
        [HttpPost]
        [Route("validar-segunda-via/")]
        public async Task<IActionResult> ValidateInfo(ValidateInfoViewModel validateInfoViewModel)
        {
            SubscriptionViewModel subscriptionViewModel;
            if (validateInfoViewModel.Reference != null)
                subscriptionViewModel = await subscriptionService.GetSubscriptionAsync(validateInfoViewModel.Reference);
            else
                subscriptionViewModel = await subscriptionService.GetLastFinishedSubscriptionAsync(validateInfoViewModel.FullName, validateInfoViewModel.Crmv, validateInfoViewModel.BirthDate, validateInfoViewModel.CpfClean);

            if (subscriptionViewModel == null || !subscriptionViewModel.HasCertificate) 
                return RedirectToAction("UpgradeSubscription", "Home", new { validation = false });

            var planInfoViewModel = await planService.GetPlanInfoAsync(subscriptionViewModel.PlanId.Value, subscriptionViewModel.PlanPriceTableId.Value);
            await mailService.SendAsync("Certificado", "2ª Via de Certificado - AmaisVet", await GetHtmlFromView("~/Views/Mail/Subscription/SecondVia.cshtml", new CertificateViewModel(subscriptionViewModel, planInfoViewModel)), subscriptionViewModel.Email);

            return RedirectToAction(nameof(Info), new { reference = subscriptionViewModel.Reference });
        }
        [HttpGet]
        [Route("meu-certificado/{reference?}")]
        public async Task<IActionResult> Info(string reference)
        {
            var subscriptionViewModel = await subscriptionService.GetSubscriptionAsync(reference);
