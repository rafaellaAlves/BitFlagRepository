...
namespace Services.Subscription
{
    public class SubscriptionService
    {
        ...
        public async Task UpdatePersonalInforAsync(string reference, SubscriptionViewModel model)
        {
            var subscription = await GetByReferenceAsync(reference);

            subscription.FullName = model.FullName;
            subscription.OccupationAreaId = model.OccupationAreaId;
            subscription.ProfessionalSpecialtyId = model.ProfessionalSpecialtyId;
            subscription.GraduationYear = model.GraduationYear;
            subscription.PhoneNumber = model.PhoneNumber;
            subscription.MobileNumber = model.MobileNumber;
            subscription.Email = model.Email;
            subscription.ZipCode = model.ZipCodeClean;
            subscription.Address = model.Address;
            subscription.AddressNumber = model.AddressNumber;
            subscription.AddressAdditionalInfo = model.AddressAdditionalInfo;
            subscription.Neighborhood = model.Neighborhood;
            subscription.City = model.City;
            subscription.State = model.State;
            subscription.AlteredDate = DateTime.Now;
        ...
        public async Task<ApplicationDbContext.Models.Subscription> GetLastFinishedSubscriptionByCpfAsync(string cpf)
        {
            var subscriptions = await context.Subscription.Where(x => x.Cpf == cpf.NumbersOnly() && x.HasCertificate).ToListAsync();

            return subscriptions.LastOrDefault();
        }
