...
namespace WEB.Controllers
{
    ...
    public class SubscriptionController : Controller
    {
        ...
        [HttpPost]
        [Route("validar-cpf")]
        public async Task<IActionResult> CheckSubscriptionByCpf(string cpf)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(cpf))
                {
                    TempData["ErrorMessage"] = "Por favor, preencha seu cpf.";
                    return RedirectToAction("UpdateAccount", "Home");
                }

                var subscription = await subscriptionService.GetLastFinishedSubscriptionByCpfAsync(cpf);
                if (subscription != null)
                    return RedirectToAction(nameof(UpdatePersonalInfo), new { reference = subscription.Reference });
            ...
            {
                TempData["ErrorMessage"] = "Erro inesperado, tente novamente mais tarde.";
                return RedirectToAction("UpdateAccount", "Home");
            }
        }
        ...
        public async Task<IActionResult> UpdatePersonalInfo(string reference)
        {
            var subscriptionViewModel = await subscriptionService.GetSubscriptionAsync(reference);
        ...
        [HttpPost]
        [Route("salvar-dados")]
        public async Task<IActionResult> _UpdatePersonalInfo(string reference, SubscriptionViewModel model)
        {
            try
            {
                await subscriptionService.UpdatePersonalInforAsync(reference, model);
                ...
                await mailService.SendAsync("Contato", "Atualização Cadastral - AmaisVet", await GetHtmlFromView("~/Views/Mail/Subscription/UpdateAccount.cshtml", new CertificateViewModel(subscriptionViewModel, planInfoViewModel)), subscriptionViewModel.Email);
                TempData["UpdateSucces"] = "Dados Atualizados com sucesso";
                return RedirectToAction("UpdatePersonalInfo", "Subscription", new { reference = reference });
            }
