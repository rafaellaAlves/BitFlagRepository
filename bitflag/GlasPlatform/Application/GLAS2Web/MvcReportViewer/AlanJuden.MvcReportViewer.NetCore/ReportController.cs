...
namespace AlanJuden.MvcReportViewer
{
    public abstract class ReportController : Controller
    {
        ...
        protected abstract void SetControlledParameters();
        public List<SimpleParameter> ControlledParameters = new List<SimpleParameter>();

        protected virtual string ReportImagePath
        {
            get
            {
                return Url.Action("ReportImage") + "?originalPath={0}";
            }
        }
        ...
        public JsonResult ViewReportPage(string reportPath, int? page = 0)
        {
            ...
            var content = this._changeReportServerUrlToJavaScriptFunctionInReportAction(model.Encoding.GetString(contentData.ReportData));
        ...
        protected ReportViewerModel GetReportViewerModel(HttpRequest request)
        {
            ...
            foreach (var simpleParameter in this.ControlledParameters)
            {
                if (!model.Parameters.ContainsKey(simpleParameter.Name)) { 
                    model.Parameters.Add(simpleParameter.Name, new string[] { string.Join(";", simpleParameter.Values) });
                }

                if (model.Parameters[simpleParameter.Name].Length == 0)
                    model.Parameters[simpleParameter.Name] = new string[] { string.Join(";", simpleParameter.Values) };
                else if (model.Parameters[simpleParameter.Name].Length == 1)
                {
                    List<string> allowedValues = new List<string>();
                    foreach(var value in model.Parameters[simpleParameter.Name][0].Split(';'))
                        if (simpleParameter.Values.Contains(value)) allowedValues.Add(value);
                    model.Parameters[simpleParameter.Name] = new string[] { string.Join(";", allowedValues) };
                }
            }
        ...
        private string _changeReportServerUrlToJavaScriptFunctionInReportAction(string content)
        {

            HtmlDocument htmlDocument = new HtmlDocument();
            htmlDocument.LoadHtml(content);

            var items = htmlDocument.DocumentNode.Descendants();
            if (items == null) return content;
            foreach (var item in items.ToList())
            {
                if (item.NodeType != HtmlNodeType.Element) continue;
                if (!item.Name.ToUpper().Equals("A") && !item.Name.ToUpper().Equals("AREA")) continue;
                if (!item.Attributes.Contains("href")) continue;

                var _href = item.Attributes["href"].Value.Replace("?", string.Empty).Replace(this.ReportServerUrl.TrimEnd('/'), Url.Action("ViewReportPage?page=1&reportPath=").TrimEnd('/'));

                item.Attributes["href"].Value = "javascript:redirectToReport('" + _href + "');";
