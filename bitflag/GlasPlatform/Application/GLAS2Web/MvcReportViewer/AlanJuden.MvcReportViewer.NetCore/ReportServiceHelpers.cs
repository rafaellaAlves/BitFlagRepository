...
namespace AlanJuden.MvcReportViewer
{
	public static class ReportServiceHelpers
	{
		...
		public static string ReplaceImageUrls(ReportViewerModel model, string reportContent)
		{
            ...
            if (reportServerDomainUri.Port != 80)
                searchForUrl += ":" + reportServerDomainUri.Port + "/";
            else
                searchForUrl += "/";

            //replace image urls with image data instead due to having issues accessing the images as a different authenticated user
            var imagePathIndex = reportContent.ToUpper().IndexOf(searchForUrl.ToUpper());
			while (imagePathIndex > -1)
			{
				var endIndex = reportContent.ToUpper().IndexOf("\"", imagePathIndex + 5);   //account for the length of src="
				if (endIndex > -1)
				{
				...
				imagePathIndex = reportContent.ToUpper().IndexOf(searchForUrl.ToUpper(), imagePathIndex + 5);
			}
