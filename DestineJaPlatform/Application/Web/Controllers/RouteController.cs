...
namespace Web.Controllers
{
    ...
    public class RouteController : Shared.BaseCRUDController<Route, RouteViewModel, int>
    {
        ...
        public async Task<IActionResult> GetClientsByAddress(List<DTO.Shared.AddressBaseViewModel> address, int? clientType, bool activeClient)
        {
            var r = await routeServices.GetContractAndServiceAvailbleCities(address);

            if (activeClient) r = r.Where(x => x.ContractId.HasValue || x.ServiceId.HasValue).ToList();
            if (clientType.HasValue) r = r.Where(x => (clientType == 1 && x.IsContract) || (clientType == 2 && x.IsService)).ToList();
