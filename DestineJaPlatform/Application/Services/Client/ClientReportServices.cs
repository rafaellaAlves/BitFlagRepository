...
namespace Services.Client
{
    public class ClientReportServices
    {
        ...
        public async Task<List<ClientReportGeneratorViewModel>> GetClientReportGeneratorViewModel( DateTime? startDate, DateTime? endDate, string search)
        {
            using var command = context.Database.GetDbConnection().CreateCommand();

            command.CommandText = "pr_ClientReport_GetGenerator  @StartDate = @_StartDate, @EndDate = @_EndDate, @Search = @_Search";
            command.Parameters.Add(new SqlParameter("@_StartDate", startDate ?? (object)DBNull.Value));
            command.Parameters.Add(new SqlParameter("@_EndDate", endDate ?? (object)DBNull.Value));
            command.Parameters.Add(new SqlParameter("@_Search", search ?? (object)DBNull.Value));

            await context.Database.OpenConnectionAsync();

            var viewModels = new List<ClientReportGeneratorViewModel>();
            using var result = await command.ExecuteReaderAsync();
            while (result.Read())
            {
                viewModels.Add(new ClientReportGeneratorViewModel
                {

                    CompanyName = result["CompanyName"].GetStringFromDbDataReader(),
                    TradeName = result["TradeName"].GetStringFromDbDataReader(),
                    Name = result["Name"].GetStringFromDbDataReader(),
                    FullName = result["FullName"].GetStringFromDbDataReader(),
                    Role = result["Role"].GetStringFromDbDataReader(),
                    Phone = result["Phone"].GetStringFromDbDataReader(),
                    MobilePhone = result["MobilePhone"].GetStringFromDbDataReader(),
                    Email = result["Email"].GetStringFromDbDataReader(),
                    HasContract = result["HasContract"].GetBoolFromDbDataReader().Value,
                    HasService = result["HasService"].GetBoolFromDbDataReader().Value,
                    LastCollectedDate = result["LastCollectedDate"].GetDateTimeFromDbDataReader(),
                }); 
            }
