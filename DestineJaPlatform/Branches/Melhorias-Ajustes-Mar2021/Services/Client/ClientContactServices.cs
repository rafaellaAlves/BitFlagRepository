...
namespace Services.Client
{
    public class ClientContactServices : Shared.BaseServices<ClientContact, ClientContactViewModel, int>
    {
        ...
        public ClientContactServices(ApplicationDbContext.Context.ApplicationDbContext context, AspNetIdentityDbContext.ApplicationDbContext identityContext, UserManager<AspNetIdentityDbContext.User> userManager, ClientContactTypeServices clientContactTypeServices) : base(context, identityContext, "ClientContactId")
        {
        ...
        public bool Exist(ServiceViewModel model, out ClientContactViewModel contact)
        {
            ...
            var entity = this.dbSet.FirstOrDefault(x => x.Name.Replace(" ", string.Empty) == contractName && x.Phone.Replace(" ", string.Empty) == contactPhone && x.Email.Replace(" ", string.Empty) == contactEmail);

            if (entity != null)//For Update Contact Data
                contact = entity.CopyToEntity<ClientContactViewModel>();

            var clientContactTypeId = clientContactTypeServices.GetDataByExternalCode("ACOMPANHARCOLETA")?.ClientContactTypeId;
        ...
        public async Task<int> GetClientIdByMainContact(int? id)
        {
            int? clientId = id;
