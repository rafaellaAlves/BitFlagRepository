...
namespace Web.Controllers
{
    ...
    public class ContractController : Shared.BaseCRUDController<Contract, ContractViewModel, int>
    {
...
#if DEBUG
                await SendContractEmail(id, contract.ContactEmail, "", "joao.gregorio@bitflag.systems;sarah.reggiani@bitflag.systems");
#else
                await SendContractEmail(id, contract.ContactEmail, "", "operacional@destineja.com.br");
#endif
        ...
        public async Task<IActionResult> SendContractEmail(int contractId, string to, string replyTo, string bcc)
        {
            ...
            LinkedResource assin3 = new LinkedResource(Path.Combine(Directory.GetCurrentDirectory(), "SystemArchives", "Assinaturas", "Assin_Ricardo.jpeg"), MediaTypeNames.Image.Jpeg) { ContentId = "Assin_Ricardo" };
            ...
            var r = emailServices.Send(
                ...
                new List<System.Net.Mail.LinkedResource> { pic, assin1, assin2, assin3 },
                false);
...
#if DEBUG
            emailServices.Send($"Contrato Assinado - Contrato \"{model.Code}\" - {client.Name}.", html_Admin, new List<string> { "sarah.reggiani@bitflag.systems", "joao.gregorio@bitflag.systems" });
#else
            emailServices.Send($"Contrato Assinado - Contrato \"{model.Code}\" - {client.Name}.", html_Admin, new List<string> { "comercial@destineja.com.br",  "financeiro@destineja.com.br", "operacional@destineja.com.br" });
#endif
            #endregion
        }
