...
#if DEBUG
            emailServices.Send($"{client.Name}, sua franquia de peso excedeu.", html_Client, null);
#else
...
#if DEBUG
            emailServices.Send($"{client.Name}, franquia excedida.", html_Admin, new List<string> { "sarah.reggiani@bitflag.systems" });
#else
            emailServices.Send($"{client.Name}, franquia excedida.", html_Admin, new List<string> { "comercial@destineja.com.br", "financeiro@destineja.com.br" });
#endif
            #endregion
        }
...
#if DEBUG
            emailServices.Send($"MTR de coleta \"{demand.DestinaJaDemandId}\" criado. Veja o resumo...", html_Admin, new List<string> { "sarah.reggiani@bitflag.systems" });
#else
...
#if DEBUG
                emailServices.Send($"{client.Name}, sua coleta foi finalizada.", html, null);
#else
                emailServices.Send($"{client.Name},  coleta foi finalizada.", html, new List<string> { client.CentralEmail });
#endif
            }
        }
        #endregion
